...
function overroom(data){
    ...
    img.onload = function(){
        ...
        ctx.fillText("房间号:"+sj.id, 175,280);
        ctx.fillText(time1, 375, 280);
        ctx.fillText(sj.zjs+"局", 575, 280);
        ...
        for(var i=0;i<sj.user.length;i++){
            ...
            ctx.fillText(decode64(user.nickname).substring(0,10), 219, 420+(i*55));
            if(user.dqjf>0){
                user.dqjf='+'+user.dqjf;
            }else{
                ctx.fillStyle = "#fff";
                ctx.fillText(decode64(user.nickname).substring(0,10), 219, 420+(i*55));
                ctx.fillText(user.dqjf, 581, 420+(i*55));
            }
            ctx.fillText( user.dqjf, 581, 420+(i*55));
        }
